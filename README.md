...
DailyTrackingSystem/
├── Context/
│   └── AppDbContext.cs          # Veritabanı bağlam sınıfı (DbContext)
├── Controllers/
│   ├── HomeController.cs        # Dashboard istatistikleri ve ana sayfa
│   ├── CategoryController.cs    # Kategori CRUD işlemleri
│   └── SpendingController.cs    # Harcama CRUD işlemleri
├── Models/
│   ├── Category.cs              # Kategori entity
│   ├── Spending.cs              # Harcama entity
│   ├── DashboardViewModel.cs    # Dashboard için ViewModel
│   └── ErrorViewModel.cs        # Hata sayfası modeli
├── Views/
│   ├── Home/
│   │   └── Index.cshtml         # Dashboard sayfası
│   ├── Category/
│   │   ├── Index.cshtml         # Kategori listesi
│   │   ├── CreateCategory.cshtml
│   │   └── UpdateCategory.cshtml
│   ├── Spending/
│   │   ├── Index.cshtml         # Harcama listesi
│   │   ├── CreateSpending.cshtml
│   │   └── UpdateSpending.cshtml
│   └── Shared/
...
Projede iki ana tablo var ve aralarında **one-to-many** ilişki kurdum:

```
Category (1) ──────── (N) Spending
   ├── Id (PK)              ├── Id (PK)
   ├── Name                 ├── Description
   ├── Icon                 ├── Amount
   └── Status               ├── Date
...
- Bir kategorinin birden fazla harcaması olabiliyor.
- `Status` alanı ile kategorileri aktif/pasif yapabiliyorsun.
...
1. Repoyu klonla:
```bash
git clone https://github.com/KULLANICI_ADIN/DailyTrackingSystem.git
```

2. `appsettings.json` içindeki connection string'i kendi SQL Server bilgilerinle güncelle:
```json
"ConnectionStrings": {
  "DefaultConnection": "Server=.\\SQLEXPRESS;Database=DailyTrackingDb;Trusted_Connection=True;TrustServerCertificate=True;"
}
```

3. Migration'ları uygula ve veritabanını oluştur:
```bash
